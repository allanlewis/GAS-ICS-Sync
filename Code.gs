...
var email = "allanlewis99@gmail.com"; // OPTIONAL: If "emailSummary" is set to true or you want to receive update notifications, you will need to provide your email address
...
function startSync() {
  ...
  for (var calendar of sourceCalendars) {
    ...
    Logger.log(
      "Syncing " + responses.length + " calendars to " + targetCalendarName,
    );
    ...
    if (
      ...
      for (var i = 0; i < calendarEvents.length; i++) {
        if (calendarEvents[i].extendedProperties != null) {
          calendarEventsIds[i] =
            calendarEvents[i].extendedProperties.private["rec-id"] ||
            calendarEvents[i].extendedProperties.private["id"];
          calendarEventsMD5s[i] =
            calendarEvents[i].extendedProperties.private["MD5"];
        }
      }
    ...
    Logger.log(
      "Processing " + recurringEvents.length + " Recurrence Instances!",
    );
  ...
  if (
    addedEvents.length + modifiedEvents.length + removedEvents.length > 0 &&
    emailSummary
  ) {
    sendSummary();
  }
  ...
  return ContentService.createTextOutput(
    JSON.stringify({ addedEvents, modifiedEvents, removedEvents }),
